...
static uint32_t SND_DEVICE_UNINIT=-1;
...
AudioHardware::AudioHardware() :
    ...
    mOutput(0), mSndEndpoints(NULL), mCurSndDevice(-1), mDualMicEnabled(false), mFmRadioEnabled(false)
{
...
static int msm72xx_enable_postproc(bool state)
{
    ...
    if (snd_device == SND_DEVICE_UNINIT) {
        LOGE("Enabling/Disabling post proc features for device: %d", snd_device);
        return -EINVAL;
    }

    if(snd_device == SND_DEVICE_SPEAKER || snd_device == SND_DEVICE_SPEAKER_IN_CALL)
    {
        device_id = 0;
...
status_t AudioHardware::setVoiceVolume(float v)
{
    ...
    LOGI("Setting in-call volume to %d (available range is 0 to 8)\n", vol);

    if ((mCurSndDevice != SND_DEVICE_UNINIT) && ((mCurSndDevice == SND_DEVICE_TTY_HEADSET) || (mCurSndDevice == SND_DEVICE_TTY_VCO))) {
        vol = 1;
...
status_t AudioHardware::setMasterVolume(float v)
{
    ...
    set_volume_rpc(SND_DEVICE_FM_HEADSET, SND_METHOD_VOICE, 21, m7xsnddriverfd);
    set_volume_rpc(SND_DEVICE_FM_SPEAKER, SND_METHOD_VOICE, 21, m7xsnddriverfd);
    ...
    set_volume_rpc(SND_DEVICE_BT,      SND_METHOD_VOICE, lrint(vol * 2), m7xsnddriverfd);
    ...
    set_volume_rpc(SND_DEVICE_HEADSET_STEREO, SND_METHOD_VOICE, lrint(vol * 2), m7xsnddriverfd);
    set_volume_rpc(SND_DEVICE_IN_S_SADC_OUT_HANDSET, SND_METHOD_VOICE, lrint(vol * 2), m7xsnddriverfd);
    set_volume_rpc(SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE, SND_METHOD_VOICE, lrint(vol * 2), m7xsnddriverfd);
...
//    LOGE("doRouting CALLED, outputDevices = 0x%x, mFmRadioEnabled = %d",outputDevices, mFmRadioEnabled);
    ...
    if (new_snd_device == SND_DEVICE_UNINIT) {
    ...
    if (new_snd_device != SND_DEVICE_UNINIT && new_snd_device != mCurSndDevice) {
...
ssize_t AudioHardware::AudioStreamInMSM72xx::read( void* buffer, ssize_t bytes)
{
    ...
    while (count > 0) {
        ...
        if (bytesRead > 0) {
            ...
            LOGI("Bytes Read = %zd ,Buffer no longer sufficient", static_cast<signed size_t>(bytesRead));
            break;
        } else {
